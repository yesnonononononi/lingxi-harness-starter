...
import com.summit.core.internalUtils.plan.PlanDirectives;
...
import com.summit.runtime.coreTools.plan.PlanApprovalWaiter.PlanApprovalResult;
...
public class DefaultPlanLoopHook implements PlanLoopHook {
    ...
    public PlanTurnResult afterToolTurn(List<ToolExecuteResult> toolResults, Execution execution,
        ...
        // acceptance or attach tips in the meantime. Every branch therefore has to work on the revision
        // that is stored *now*: acting on the pre-wait snapshot would both hide those edits from the
        // model (the directive renders the plan it is given) and, for the approval path, write the
        // stale snapshot back over the user's revision.
