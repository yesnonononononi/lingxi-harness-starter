    ...
    public static String approvedPlanMessage(Plan plan) {
        return """
                ...
                - the approved plan below is the authoritative revision: the user may have edited task
                  descriptions or acceptance criteria, and may have attached `user tips`, while
                  reviewing it, so re-read it instead of relying on the draft you wrote earlier;
                - work task by task; honour every task's acceptance criterion, its dependencies and its
                  `user tips` — a tip is an extra requirement the user added on purpose, so the task is
                  not done until the tip is satisfied too;
