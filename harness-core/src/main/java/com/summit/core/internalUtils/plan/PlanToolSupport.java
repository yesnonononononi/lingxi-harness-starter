...
import com.summit.core.internalUtils.plan.PlanToolRequests.TaskDraft;
...
public final class PlanToolSupport {
     ...
     *
     * <p>{@code tips} stays {@code null}: it is a human-side addition and never comes from the model,
     * and {@code Plan#upsertTask} carries the stored tips over when the draft replaces a known id.</p>
     */
    public static Task taskFromDraft(TaskDraft draft) {
        ...
        return new Task(draft.id(), draft.title(), draft.description(), TaskStatus.parse(draft.status()),
                draft.dependencies(), draft.priority(), draft.acceptance(), null);
    }
    ...
    public static TaskPatch patchOf(TaskDraft draft) {
        // tips is deliberately not mapped: the model never writes the user's own hints.
    ...
    public static Task applyPatch(Task task, TaskPatch patch) {
        ...
        if (patch.tips() != null) {
            result = result.withTips(patch.tips());
        }
        return result;
    }
